...
class LandCoverAIDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, img_dir, crop_size, upscale_factor, use_aug=None):
        super().__init__()
        self.filenames_hr = sorted([Path(x) for x in Path(img_dir).iterdir() if not '_m' in x.stem])[:10]
...
if __name__ == "__main__":
    ...
    for idx, batch in enumerate(train_loader):
        lr, hr, an = batch
        hr = hr.numpy()
        rnd = np.random.uniform(size=hr.shape)

        mse1 = ((hr - rnd) ** 2).mean() * bs

        mse2 = ((hr - rnd) ** 2).sum() / (256*256*3)
        ...
        #     axes[i, 2].imshow(rnd[i].transpose(1,2,0))
        # plt.show()